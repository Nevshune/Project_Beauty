...
// 선택된 강의의 정보를 가져옴
$sql = "SELECT * FROM contest_list WHERE Num = $num";
...
<?php include './nav.php'; ?>
  ...
  <div class="w-full h-full flex flex-col items-center justify-center px-20">
    ...
    <form action="./process/contest_update_process.php" method="POST" enctype="multipart/form-data" class="w-full">
      ...
      <div class="w-full h-24 flex items-center">
        ...
        <input type="text" name="contestName" class="w-[50%]" value="<?php echo $row['contestName']; ?>" required>
      </div>
      <div class="w-full h-60 flex items-center">
        ...
        <textarea name="contestSub" class="w-[75%] h-[90%]" required><?php echo $row['contestSub']; ?></textarea>
      </div>
      <div class="w-full h-full flex items-center border-b-2 border-white">
        ...
        <input type="hidden" name="existingThumbnail" value="<?php echo $row['contestImg']; ?>">

        <!-- 선택된 이미지를 보여주는 부분 -->
        <div class="w-50% h-[300px] mr-4">
          <img id="preview-image" class="h-full" src="./uploads/contest/<?php echo $row['contestImg']; ?>" alt="선택된 이미지">
        </div>

        <!-- 파일 선택 버튼 -->
        <div class="w-50%">
          <input type="file" name="contestImg" class="w-[100%]" title="파일 변경" onchange="previewFile()">
      ...
      <div class="w-full h-24 flex items-center">
        ...
        <div class="flex items-center">
          <input id="checkbox" type="checkbox" <?php echo ($row['contestExpose'] == 1) ? 'checked' : ''; ?> name="classExpose" class="mr-2">
  ...
  <script>
    ...
    function previewFile() {
      ...
      if (file) {
        ...
        preview.src = "./uploads/contest/<?php echo $row['contestImg']; ?>";
      }
    }
  </script>
